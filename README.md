...
The application runs entirely on a **Live Backend Mode** connecting a Vanilla JS frontend to a FastAPI/PostgreSQL (Supabase) server.
...
2. **PostgreSQL Database** (e.g., Supabase): Required for live backend database storage.
3. **Web Browser**: Google Chrome, Mozilla Firefox, Microsoft Edge, or Safari to run the frontend interface.
...
1. Create a PostgreSQL database (e.g., using Supabase, Neon, or local PGAdmin).
2. Navigate to the `backend/` directory:
   ```bash
   cd backend
   ```
3. Copy the `.env.example` file to create a new `.env` file and insert your connection string:
   ```env
...
### 14. Swagger API Documentation (Fund & Transaction Endpoints)
Complete list of Fund and Transaction CRUD REST API routes:
![Swagger API Documentation Bottom](screenshots/swagger_docs_bottom.png)
